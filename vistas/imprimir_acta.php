...
if((($id_cita != 0 and $tipo_paciente_citas != '') and (($tipo_paciente_citas=='Designado' or $tipo_paciente_citas=='Conyuge') and $vet_titular=="")) or ($id_cita==0 and $data_orden[0]['institucion']=='CONYUGE')){
  ...
  if($dataOrdenSinCita['institucion'] != "" and $dataOrdenSinCita['titular'] != "" and $dataOrdenSinCita['dui_titular'] != ""){
    ...
    //Cabecera mensage
    $msj_main = "<b> $depto_acta, $sucursal </b> a las $hora horas del dia $hoy se hace constar la entrega formal a :<br> <b style='text-transform:uppercase'>$paciente</b> con DUI: $dui_acta del sector $t_paciente(Segun SIVET Web).<br>";
    //Subtitle

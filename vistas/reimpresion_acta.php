...
//Tipo de receptor segun Registros
$isTerceroReceptor = ($data_acta_actual[0]['tipo_receptor'] == 'tercero') ? 'Si' : 'No';
$data_sucursal = $reportes->getDataSucursal($sucursal);
//New datos para actas sin cita
$dataOrdenSinCita = $reportes->getPacientesActasConyuge($dui_acta);
//Fecha de reimpresion 
$hoy = $dataOrdenSinCita['fecha_impresion'];
$hora = $dataOrdenSinCita['hora_impresion'];
...
if($isTerceroReceptor && $data_acta_actual[0]['receptor'] != "" && $data_acta_actual[0]['dui_receptor'] != ""){
  $receptor = strtoupper($data_acta_actual[0]['receptor']);
  $dui_receptor = $data_acta_actual[0]['dui_receptor'];
...
if((($id_cita != 0 and $tipo_paciente_citas != '') and (($tipo_paciente_citas=='Designado' or $tipo_paciente_citas=='Conyuge') and $vet_titular=="")) or ($id_cita==0 and $data_orden[0]['institucion']=='CONYUGE')){
   //New code
  //Fecha imple::: 27-02-2023
  //Validacion si estos campos estan vacios
  if($dataOrdenSinCita['institucion'] != "" and $dataOrdenSinCita['titular'] != "" and $dataOrdenSinCita['dui_titular'] != ""){
    $institucion_vet = $dataOrdenSinCita['institucion'];
    $nombre_titular= $dataOrdenSinCita['titular'];
    $dui_titular_vet = $dataOrdenSinCita['dui_titular'];
    $fecha_impresion = $dataOrdenSinCita['fecha_impresion'];
    $hora_impresion = $dataOrdenSinCita['hora_impresion'];
    //Cabecera mensage
    $msj_main = "<b> $depto_acta, $sucursal </b> a las $hora_impresion horas del dia $fecha_impresion se hace constar la entrega formal a :<br> <b style='text-transform:uppercase'>$paciente</b> con DUI: $dui_acta del sector $t_paciente(Segun SIVET Web).<br>";
    //Subtitle
    $msj_beneficiario = "Recibe el beneficio de $institucion_vet de: $nombre_titular y DUI: $dui_titular_vet (Segun SIVET Web).
    ";
  }else{
    $msj_main = "";
    $msj_beneficiario = "";
    echo "<h2>POR FAVOR,DEBE ACTUALIZAR LOS DATOS DE ESTE PACIENTE</h2>";
  }
...
$tel = explode('-',$tel_suc); echo $tel[1]*/ echo "7743-6201";
...
<span><b>FECHA:</b> <?php echo $hoy." ".$hora; //$dateTime;?></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span><b>SUCURSAL DE ÓPTICA: </b><?php echo $sucursal;?></span> <br><br><br>
...
if($dataOrdenSinCita['institucion'] != "" and $dataOrdenSinCita['titular'] != "" and $dataOrdenSinCita['dui_titular'] != ""){
  ...
  echo "<b>NOMBRE DEL TITULAR:</b> $nombre_titular <br><br><br>
 
  <b>DUI DEL TITULAR:</b> $dui_titular_vet           <b style='margin-left:25px'>SECTOR:</b> $institucion_vet <br><br><br>";
